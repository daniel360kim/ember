...
from vehicle.rigid_body import Derivative, RigidBody
...
class Atlas(BaseVehicle):
    ...
    def dynamics(self, X: torch.tensor, U: torch.tensor, t: torch.tensor):
        ...
        # Net force
        force_world = force_gravity + sum(wrench.force_world for wrench in wrenches)
        ...
        accel_clamped = torch.clamp(deriv.acceleration[..., 2], min=0.0)
        on_ground = (position[..., 2] <= 0) & (velocity[..., 2] <= 0)
        accel_z_clamped = torch.where(on_ground, accel_clamped, deriv.acceleration[..., 2])
        
        deriv.acceleration[..., 2] = accel_z_clamped
        
        # Gimbal dynamics
        gimbal_delta = self.gimbal.dynamics(gimbal_state=gimbal_angle, torque_cmd=U, cg=cg, t=t)
        
        return torch.cat((deriv.velocity, deriv.quat_deriv, deriv.acceleration, deriv.angular_acceleration, gimbal_delta), dim=-1)
